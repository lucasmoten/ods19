...
+ Request (application/json)
    ...
    + recursiveSharing (boolean, optional) - If set to true, updates to sharing and permissions are applied to an objects children. Note that this initiates an asyncrhronous operation on the server, in the background. Clients may need to wait for changes to take effect.
