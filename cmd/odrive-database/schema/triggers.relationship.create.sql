delimiter //
SELECT 'Creating trigger ti_relationship' as Action
//
...
END
//
SELECT 'Creating trigger tu_relationship' as Action
//
...
END
//
SELECT 'Creating trigger td_relationship' as Action
//
CREATE TRIGGER td_relationship
BEFORE DELETE ON relationship FOR EACH ROW
BEGIN
	DECLARE error_msg varchar(128) default 'Deleting records are not allowed. Use isDeleted, deletedDate, and deletedBy';
	signal sqlstate '45000' set message_text = error_msg;
END
//
SELECT 'Creating trigger td_a_relationship' as Action
//
