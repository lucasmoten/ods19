source table.acm.create.sql
source table.acmkey.create.sql
source table.acmpart.create.sql
source table.acmvalue.create.sql
source table.dbstate.create.sql
source table.field_changes.create.sql
source table.object.create.sql
source table.objectacm.create.sql
source table.object_permission.create.sql
source table.object_property.create.sql
source table.object_tag.create.sql
source table.object_type.create.sql
source table.object_type_property.create.sql
source table.property.create.sql
source table.relationship.create.sql
source table.user.create.sql
source table.user_object_favorite.create.sql
source table.user_object_subscription.create.sql

