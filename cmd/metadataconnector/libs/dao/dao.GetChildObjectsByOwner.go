...
// user or group.
func (dao *DataAccessLayer) GetChildObjectsByOwner(
	...
	query := `select sql_calc_found_rows o.*, ot.name typeName from object o
  inner join object_type ot on o.typeid = ot.id
  where o.isdeleted = 0 and o.parentid = ? and o.ownedby = ?`
	...
	err := dao.MetadataDB.Select(&response.Objects, query, object.ID, owner)
	if err != nil {
		print(err.Error())
	}
